        ...
        <div className="h-100 p-3 d-flex flex-column justify-content-between">
          <ul className="nav nav-pills flex-column">
            <li className="nav-item fs-5">
              <Link to="/" className={`nav-link p-3 mb-3 ${activeLink === 'Home' ? 'active' : ''}`} aria-current="page" onClick={() => activeHandler('Home')}>
                <i className="nav-link-icon d-inline-block fa-solid fa-house me-3 fs-4"></i>
                Home
              </Link>
              <Link to="/" className={`nav-link p-3 mb-3 ${activeLink === 'Search' ? 'active' : ''}`} aria-current="page" onClick={() => activeHandler('Search')}>
                <i className="nav-link-icon d-inline-block fa-solid fa-magnifying-glass me-3 fs-4"></i>
                Search
              </Link>
              <Link to="/" className={`nav-link p-3 mb-3 ${activeLink === 'Explore' ? 'active' : ''}`} aria-current="page" onClick={() => activeHandler('Explore')}>
                <i className="nav-link-icon d-inline-block fa-regular fa-compass me-3 fs-4"></i>
                Explore
              </Link>
              <Link to="/" className={`nav-link p-3 mb-3 ${activeLink === 'Messages' ? 'active' : ''}`} aria-current="page" onClick={() => activeHandler('Messages')}>
                <i className="nav-link-icon d-inline-block fa-solid fa-comments me-3 fs-4"></i>
                Messages
              </Link>
              <Link to="/" className={`nav-link p-3 mb-3 ${activeLink === 'Create' ? 'active' : ''}`} aria-current="page" onClick={() => activeHandler('Create')}>
                <i className="nav-link-icon d-inline-block fa-regular fa-square-plus me-3 fs-4"></i>
                Create
              </Link>
              <Link to="/" className={`nav-link p-3 mb-3 ${activeLink === 'Profile' ? 'active' : ''}`} aria-current="page" onClick={() => activeHandler('Profile')}>
